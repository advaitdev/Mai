...
public class PatheticNavigator {
    ...
    public void setWalkableTarget(Location target) {
        ...
        AtomicReference<Path> lastPath = new AtomicReference<>();
        ...
        npc.getCitizensNPC().getNavigator().getDefaultParameters().addRunCallback(() -> {
            ...
            if (counter.get() == 20) {
                AGENT.getGroundPath(citizensNPC.getStoredLocation(), target).thenAccept(result -> {
                    if (result.successful() && !PatheticUtil.isSubpathEquivalent(result.getPath(), lastPath.get())) {
                        Bukkit.getScheduler().runTask(PatheticCitizens.getInstance(), () -> {
                            ...
                            for (Player player : Bukkit.getOnlinePlayers()) player.sendActionBar(Component.text("Pathetic runCallback ran at " + System.currentTimeMillis()));
